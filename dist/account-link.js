...
exports.initAccountLinkSchema = initAccountLinkSchema;
exports.registerAccountLink = registerAccountLink;
exports.canonicalChatId = canonicalChatId;
...
async function initAccountLinkSchema() {
...
async function progressOf(chatId, db = db_1.pool) {
    const { rows } = await db.query(`SELECT total_earned FROM clicker_state WHERE chat_id = $1`, [chatId]);
    return Number(rows[0]?.total_earned ?? 0);
}
async function registerAccountLink(chatId, phone) {
    ...
    try {
        ...
        const linked = await client.query(`SELECT canonical_chat_id FROM account_links WHERE alias_chat_id=$1`, [chatId]);
        const candidate = Number(linked.rows[0]?.canonical_chat_id ?? chatId);
        const cur = await client.query(`SELECT canonical_chat_id FROM phone_canonical WHERE phone = $1 FOR UPDATE`, [phone]);
        if (cur.rowCount === 0) {
            ...
            await client.query(`INSERT INTO phone_canonical (phone, canonical_chat_id) VALUES ($1, $2)`, [phone, candidate]);
            await client.query("COMMIT");
            _cache.delete(chatId);
            _cache.delete(candidate);
        ...
        await client.query(`INSERT INTO account_links (alias_chat_id, canonical_chat_id, phone)
       VALUES ($1, $2, $3)
       ON CONFLICT (alias_chat_id) DO UPDATE SET canonical_chat_id = $2, phone = $3, linked_at = NOW()`, [alias, canonical, phone]);
        // Прежние алиасы этого телефона перенаправляем на нового канона
        const moved = await client.query(`UPDATE account_links SET canonical_chat_id = $1 WHERE phone = $2 AND alias_chat_id <> $1
       RETURNING alias_chat_id`, [canonical, phone]);
        // Канон не может сам быть алиасом
        await client.query(`DELETE FROM account_links WHERE alias_chat_id = $1`, [canonical]);
        await client.query(`UPDATE phone_canonical SET canonical_chat_id = $1, updated_at = NOW() WHERE phone = $2`, [canonical, phone]);
        await client.query("COMMIT");
        ...
        (0, analytics_1.trackEvent)(canonical, "account_link", { alias, phone_last4: phone.slice(-4) });
        logger_1.log.info({ canonical, alias }, "[account-link] linked");
        return { linked: true, canonicalChatId: canonical, aliasedChatId: alias };
    }
    catch (e) {
        await client.query("ROLLBACK").catch(() => { });
        logger_1.log.error({ err: e, chatId }, "[account-link]");
...
async function canonicalChatId(chatId) {
    const hit = _cache.get(chatId);
    if (hit && Date.now() - hit.ts < CACHE_TTL)
        return hit.canon;
    let canon = chatId;
    const { rows } = await db_1.pool.query(`SELECT canonical_chat_id FROM account_links WHERE alias_chat_id = $1`, [chatId]);
    if (rows[0])
        canon = Number(rows[0].canonical_chat_id);
    const now = Date.now();
    _cache.set(chatId, { canon, ts: now });
    if (_cache.size > CACHE_MAX) {
        for (const [id, item] of _cache)
...
async function linksOf(chatId) {
    const { rows } = await db_1.pool.query(`SELECT alias_chat_id, phone FROM account_links WHERE canonical_chat_id = $1`, [chatId]);
