...
bot.command("start", async (ctx) => {
    ...
    if (ctx.from && !(await (0, club_2.isPhoneVerified)(ctx.from.id).catch(() => false))) {
        await ctx.reply("Для заданий за покупки в физических точках привяжи номер телефона:", { reply_markup: new grammy_1.Keyboard().requestContact("📱 Поделиться телефоном").resized().oneTime() }).catch(() => { });
    }
});
