...
async function pushHolidayPreorder() {
    const due = (0, holidays_1.getHolidaysToPushToday)();
    if (due.length === 0)
        return;
    ...
    for (const occ of due) {
        ...
        for (const { chat_id } of subs) {
            ...
            if (await (0, db_1.hasHolidayPushSent)(chat_id, occ.holiday.id, occ.year).catch(() => false)) {
                skipped++;
                continue;
            }
            ...
            if (ok) {
                sent++;
                await (0, db_1.markHolidayPushSent)(chat_id, occ.holiday.id, occ.year).catch(() => { });
            }
        }
...
async function checkOrderStatusChanges() {
    ...
    for (const s of subs) {
        ...
        if (recent.length === 0)
            continue;
        const seen = await (0, db_1.getOrderStatusMap)(s.chat_id);
        for (const o of recent) {
            ...
            if (prev === undefined) {
                ...
                await (0, db_1.setOrderStatus)(s.chat_id, orderId, status).catch(() => { });
                continue;
            }
            if (prev === status)
                continue;
...
function runCleanup() {
    ...
    (0, selfie_cake_1.cleanupOldSelfies)().catch(() => { }); // 2 часа (внутри модуля)
}
...
async function sendPushSafely(chatId, kind, text, opts) {
    const gate = await (0, db_1.canSendNotification)(chatId, kind).catch(() => ({ ok: false, reason: "db_error" }));
    if (!gate.ok) {
    ...
    try {
        ...
        await (0, db_1.logNotification)(chatId, kind).catch(() => { });
        return true;
    }
    catch (e) {
        const msg = e.message || "";
...
app.post("/api/transcribe", rateLimit(20), express_1.default.raw({ type: ["audio/*", "application/octet-stream"], limit: "6mb" }), async (req, res) => {
...
app.post("/api/lead-corporate", rateLimit(5), express_1.default.json({ limit: "1mb" }), async (req, res) => {
    const b = req.body;
...
app.post("/api/cake-concept/generate", auth_1.requireTgUser, rateLimit(2), async (req, res) => {
...
app.post("/api/cake-concept/submit", auth_1.requireTgUser, rateLimit(5), async (req, res) => {
    const b = req.body;
...
app.post("/api/selfie-cake", auth_1.requireTgUser, rateLimit(3), express_1.default.json({ limit: "8mb" }), async (req, res) => {
    const healthy = await (0, cake_concept_1.isConceptEnabled)();
    if (!healthy) {
...
_Узнать статус: напишите боту_`;
        bot.api.sendMessage(tg.id, msg, { parse_mode: "Markdown" }).catch((e) => {
            console.warn(`[ORDER push] failed for chat ${tg.id}:`, e.message);
        });
...
const SWEET_CHECK_FILE = path_1.default.join(__dirname, "..", "data", "sweet-check-weeks.json");
let _sweetWeeksCache = null;
function loadSweetCheckWeeks() {
    if (_sweetWeeksCache)
    ...
    try {
        ...
        console.error("[sweet-check] load failed:", e.message);
    }
