...
exports.SHOP = void 0;
exports.isPetLocation = isPetLocation;
exports.settlePetNeed = settlePetNeed;
exports.effectiveCareStreak = effectiveCareStreak;
exports.initPetSchema = initPetSchema;
exports.getPet = getPet;
exports.buyPetItem = buyPetItem;
exports.equipPetItem = equipPetItem;
exports.doPetAction = doPetAction;
exports.setPetLocation = setPetLocation;
...
const SHOP_IDS = new Set(exports.SHOP.map((s) => s.id));
// падение потребностей, очков/час
const DECAY = { hunger: 6, mood: 4, energy: 3, hygiene: 2.5 };
...
const LOCATIONS = ["kitchen", "bedroom", "playroom", "yard"];
function isPetLocation(value) {
    return typeof value === "string" && LOCATIONS.includes(value);
}
const clamp = (v) => Math.max(0, Math.min(100, Math.round(v)));
const xpForNext = (level) => level * 100; // нужно опыта до следующего уровня
/** Накапливает дробный decay между чтениями, чтобы частые GET не замораживали потребность. */
function settlePetNeed(current, decayPerHour, elapsedHours, previousCarry = 0) {
...
const irkDay = (offsetDays = 0) => new Date(Date.now() + 8 * 3600 * 1000 - offsetDays * 86400000).toISOString().slice(0, 10);
/** Текущая серия для показа: пропущенный день обнуляет её ещё до нового действия. */
function effectiveCareStreak(lastCareDate, storedStreak, today = irkDay()) {
...
const careStreakBonus = (streak) => 100 * Math.min(Math.max(1, streak), 10);
async function initPetSchema() {
  ...
  `);
    ...
    await db_1.pool.query(`UPDATE pet_state SET care_streak_best = care_streak WHERE care_streak > care_streak_best`);
}
function toState(r) {
    return {
...
async function getItems(chatId, db = db_1.pool) {
...
async function getPet(chatId) {
    ...
    try {
        ...
        await (0, clicker_1.settleClickerBeforeIncomeChange)(client, chatId);
        ...
        if (hrs > 0.001) {
            for (const need of ["hunger", "mood", "energy", "hygiene"]) {
                const settled = settlePetNeed(r[need], DECAY[need], hrs, r[`${need}_carry`]);
                r[need] = settled.value;
                r[`${need}_carry`] = settled.carry;
            }
            await client.query(`UPDATE pet_state SET hunger=$2, mood=$3, energy=$4, hygiene=$5,
           hunger_carry=$6, mood_carry=$7, energy_carry=$8, hygiene_carry=$9,
           updated_at=NOW() WHERE chat_id=$1`, [chatId, r.hunger, r.mood, r.energy, r.hygiene,
...
async function buyPetItem(chatId, id) {
    const shopItem = exports.SHOP.find((s) => s.id === id);
    if (!shopItem)
    ...
    try {
        ...
        if (Number(bal.rows[0]?.balance ?? 0) < shopItem.price) {
            await client.query("ROLLBACK");
...
async function equipPetItem(chatId, id) {
    if (id && !SHOP_IDS.has(id))
        return { ok: false, reason: "bad_item" };
    await getPet(chatId);
    const client = await db_1.pool.connect();
    try {
        ...
        await (0, clicker_1.settleClickerBeforeIncomeChange)(client, chatId);
        if (id) {
            const owned = await client.query(`SELECT 1 FROM pet_items WHERE chat_id=$1 AND item=$2 FOR UPDATE`, [chatId, id]);
            if (!owned.rows.length) {
                await client.query("ROLLBACK");
...
async function doPetAction(chatId, action) {
    if (!RESTORE[action])
    ...
    try {
        ...
        for (const need of ["hunger", "mood", "energy", "hygiene"]) {
            ...
            if (r[need] === 0 || r[need] === 100)
                r[`${need}_carry`] = 0;
        }
        r.xp += XP_PER_ACTION;
        while (r.xp >= xpForNext(r.level)) {
            r.xp -= xpForNext(r.level);
            r.level += 1;
        }
        ...
        await client.query(`UPDATE pet_state SET hunger=$2,mood=$3,energy=$4,hygiene=$5,xp=$6,level=$7,
         care_streak=$8,care_date=$9,care_streak_best=$10,
         hunger_carry=$11,mood_carry=$12,energy_carry=$13,hygiene_carry=$14,
         updated_at=NOW() WHERE chat_id=$1`, [chatId, r.hunger, r.mood, r.energy, r.hygiene, r.xp, r.level, r.care_streak, r.care_date, r.care_streak_best,
            r.hunger_carry, r.mood_carry, r.energy_carry, r.hygiene_carry]);
        await (0, clicker_1.addClickerBalance)(chatId, streakBonus, client); // no-op если streakBonus<=0; атомарно в этой транзакции
        const balRow = await client.query(`SELECT balance, state_revision FROM clicker_state WHERE chat_id=$1`, [chatId]);
        await client.query("COMMIT");
        const state = toState(r);
...
async function setPetLocation(chatId, location) {
    if (!isPetLocation(location))
    ...
    await db_1.pool.query(`UPDATE pet_state SET location=$2 WHERE chat_id=$1`, [chatId, loc]);
    return { ok: true, state: await getPet(chatId) };
}
