...
router.post("/api/promo/validate", auth_1.optionalUser, (0, middleware_1.rateLimit)(20), async (req, res) => {
    const body = req.body;
    ...
    if (!sync.result.ok || !sync.promo) {
        ...
        if (r.reason === "min_order_not_met" && min > 0) {
            ...
            const tgUser2 = (0, auth_1.tryGetTgUser)(req);
            if (tgUser2) {
                try {
                    const ur = await (0, db_1.findUserReward)(tgUser2.id, code);
                    if (ur) {
                        const todayIrk = new Date(Date.now() + 8 * 3600000).toISOString().slice(0, 10);
                        if (ur.used_at) {
                            res.json({ ok: false, reason: "already_used", message: "Награда уже использована" });
                            return;
                        }
                        if (new Date(ur.expires_at).toISOString().slice(0, 10) < todayIrk) {
                            res.json({ ok: false, reason: "expired", message: "Срок действия истёк" });
                            return;
                        }
                        if (ur.reward_type !== "percent" && ur.reward_type !== "amount") {
                            res.json({ ok: false, reason: "show_at_cashier", message: "Награду выдаёт кассир — покажите код на кассе" });
                            return;
                        }
                        if (ur.min_order && cartTotal < ur.min_order) {
                            res.json({ ok: false, reason: "min_order_not_met", min_order: ur.min_order, message: `Минимальная сумма заказа: ${ur.min_order.toLocaleString("ru-RU")} ₽` });
                            return;
                        }
                    ...
                    logger_1.log.error({ err: e, code }, "[promo/validate] user_reward lookup");
                }
            }
    ...
    try {
        ...
        logger_1.log.error({ err: e, code }, "[promo/validate]");
...
router.post("/api/promo/use", auth_1.requireTgUser, (0, middleware_1.rateLimit)(10), (_req, res) => {
