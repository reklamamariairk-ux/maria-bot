...
exports.loadSweetCheckWeeks = loadSweetCheckWeeks;
exports.loadSweetCheckPrizes = loadSweetCheckPrizes;
...
function loadSweetCheckWeeks() {
    ...
    try {
        ...
        logger_1.log.error({ err: e }, "[sweet-check] weeks load failed");
    }
    _weeksCache = WEEKS_FALLBACK;
    return _weeksCache;
}
router.get("/api/sweet-check/active", (0, middleware_1.rateLimit)(60), (_req, res) => {
...
router.post("/api/admin/sweet-check/reload", middleware_1.requireAdminToken, (0, middleware_1.requireAdminRole)("operator"), (_req, res) => {
    _weeksCache = null;
    const weeks = loadSweetCheckWeeks();
    res.json({ ok: true, total: weeks.length, first: weeks[0]?.from, last: weeks.at(-1)?.to });
});
const PRIZES_FALLBACK = {
...
function loadSweetCheckPrizes() {
    ...
    try {
        ...
        logger_1.log.error({ err: e }, "[sweet-check] prizes load failed");
    }
    _prizesCache = PRIZES_FALLBACK;
    return _prizesCache;
}
router.get("/api/sweet-check/prizes", (0, middleware_1.rateLimit)(60), (_req, res) => {
    res.json(loadSweetCheckPrizes());
});
router.post("/api/admin/sweet-check-prizes/reload", middleware_1.requireAdminToken, (0, middleware_1.requireAdminRole)("operator"), (_req, res) => {
    _prizesCache = null;
