...
exports.initAppAuthSchema = initAppAuthSchema;
exports.signAppToken = signAppToken;
exports.verifyAppToken = verifyAppToken;
exports.attachAppLoginChat = attachAppLoginChat;
exports.completeAppLogin = completeAppLogin;
/**
...
const signKey = BOT_TOKEN
    ? crypto_1.default.createHmac("sha256", "MariaAppAuth").update(BOT_TOKEN).digest()
    : null;
async function initAppAuthSchema() {
...
function signAppToken(phone) {
    ...
    const sig = b64url(crypto_1.default.createHmac("sha256", signKey).update(payload).digest());
    return `${payload}.${sig}`;
}
function verifyAppToken(token) {
    ...
    if (!payload || !sig)
        return null;
    const expect = b64url(crypto_1.default.createHmac("sha256", signKey).update(payload).digest());
    if (sig.length !== expect.length || !crypto_1.default.timingSafeEqual(Buffer.from(sig), Buffer.from(expect)))
        return null;
    try {
        const data = JSON.parse(Buffer.from(payload.replace(/-/g, "+").replace(/_/g, "/"), "base64").toString());
        if (!data.p || !data.e || data.e < Date.now() / 1000)
            return null;
...
async function attachAppLoginChat(nonce, chatId) {
    const client = await db_1.pool.connect();
    try {
        await client.query("BEGIN");
        const { rowCount } = await client.query(`UPDATE app_login_nonces SET chat_id = $2
        WHERE nonce = $1 AND phone IS NULL AND created_at > NOW() - INTERVAL '${NONCE_TTL_MIN} minutes'`, [nonce, chatId]);
        if (!rowCount) {
        ...
        await client.query(`UPDATE app_login_nonces SET chat_id=NULL
        WHERE chat_id=$1 AND nonce<>$2 AND phone IS NULL`, [chatId, nonce]);
...
async function completeAppLogin(chatId, phone) {
    const clean = phone.replace(/[^\d+]/g, "");
    const { rowCount } = await db_1.pool.query(`UPDATE app_login_nonces SET phone = $2
      WHERE nonce = (
...
function startRateLimited(ip) {
    const now = Date.now();
    const arr = (startHits.get(ip) ?? []).filter((t) => now - t < 60000);
    if (arr.length >= 10) {
    ...
    if (startHits.size > 5000) {
        for (const [k, v] of startHits)
            if (!v.some((t) => now - t < 60000))
                startHits.delete(k);
        if (startHits.size > 5000)
            startHits.delete(startHits.keys().next().value);
    }
    return false;
}
router.post("/auth/start", async (req, res) => {
    if (!BOT_TOKEN) {
        res.status(503).json({ ok: false, error: "app_auth_not_configured" });
        return;
    }
    const ip = String(req.ip || req.socket.remoteAddress || "unknown");
    if (startRateLimited(ip)) {
        res.status(429).json({ ok: false, error: "rate_limited" });
        return;
    }
    const nonce = crypto_1.default.randomBytes(16).toString("hex");
    await db_1.pool.query(`INSERT INTO app_login_nonces (nonce) VALUES ($1)`, [nonce]);
    // Ленивая уборка протухших nonce (дешёвая, раз на выдачу)
    db_1.pool.query(`DELETE FROM app_login_nonces WHERE created_at < NOW() - INTERVAL '1 hour'`).catch(() => { });
    res.json({ ok: true, nonce, link: `https://t.me/${BOT_USERNAME}?start=applogin_${nonce}` });
});
router.get("/auth/poll", (0, middleware_1.rateLimit)(120), async (req, res) => {
    const nonce = String(req.query.nonce || "");
    if (!/^[a-f0-9]{32}$/.test(nonce)) {
        res.status(400).json({ ok: false, error: "bad_nonce" });
        return;
    }
    // DELETE ... RETURNING делает выдачу токена одноразовой даже при двух
    // одновременных poll-запросах.
    const completed = await db_1.pool.query(`DELETE FROM app_login_nonces
      WHERE nonce = $1 AND phone IS NOT NULL
        AND created_at > NOW() - INTERVAL '${NONCE_TTL_MIN} minutes'
      RETURNING phone`, [nonce]);
    ...
    const pending = await db_1.pool.query(`SELECT 1 FROM app_login_nonces
      WHERE nonce=$1 AND phone IS NULL
        AND created_at > NOW() - INTERVAL '${NONCE_TTL_MIN} minutes'`, [nonce]);
    if (pending.rowCount) {
...
router.get("/lk", async (req, res) => {
    ...
    if (!APP_BRIDGE_URL || !APP_BRIDGE_TOKEN) {
        res.status(503).json({ ok: false, error: "bridge_not_configured" });
        return;
    }
    try {
        ...
        const sig = crypto_1.default.createHmac("sha256", APP_BRIDGE_TOKEN).update(auth.phone + ":" + ts).digest("hex");
        ...
        res.status(502).json({ ok: false, error: "bridge_error", detail: e.message.slice(0, 120) });
    }
});
