...
function adminSystemRouter() {
    const router = (0, express_1.Router)();
    router.get("/api/admin/system/status", middleware_1.requireAdminToken, (0, middleware_1.rateLimit)(30), async (_req, res) => {
        ...
        try {
            ...
            logger_1.log.error({ err: error }, "[admin system status]");
            res.status(503).json({ ok: false, db: { ok: false }, checkedAt: new Date().toISOString() });
        }
    });
    router.get("/api/admin/system/audit", middleware_1.requireAdminToken, (0, middleware_1.rateLimit)(30), async (req, res) => {
        const limit = Math.min(200, Math.max(1, Number(req.query.limit) || 50));
        try {
            const { rows } = await db_1.pool.query(`SELECT chat_id, event, meta, created_at
           FROM clicker_events
          ...
          LIMIT $1`, [limit]);
            ...
            logger_1.log.error({ err: error }, "[admin system audit]");
            res.status(500).json({ error: "internal" });
        }
    });
    router.get("/api/admin/economy/report", middleware_1.requireAdminToken, (0, middleware_1.rateLimit)(30), async (_req, res) => {
        try {
            const [totals, cases, top, suspicious] = await Promise.all([
                db_1.pool.query(`SELECT COUNT(*)::int AS players,
                      ...
                      FROM clicker_state`),
                db_1.pool.query(`SELECT COUNT(*)::int AS openings,
                ...
                db_1.pool.query(`SELECT chat_id::text, balance::bigint, total_earned::bigint, taps::bigint,
            ...
            logger_1.log.error({ err: error }, "[admin economy report]");
            res.status(500).json({ error: "internal" });
        }
    });
    return router;
