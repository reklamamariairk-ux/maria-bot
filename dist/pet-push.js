...
exports.runPetHungryPush = runPetHungryPush;
exports.runPetEnergyPush = runPetEnergyPush;
/**
 ...
 */
async function runPetHungryPush(push) {
    const today = irkToday();
    const { rows } = await db_1.pool.query(`SELECT p.chat_id FROM pet_state p
      ...
      LIMIT $1`, [CANDIDATE_LIMIT]);
    let sent = 0;
    for (const r of rows) {
        const chatId = Number(r.chat_id);
        if (await (0, analytics_1.wasFunnelSent)(chatId, "pet_hungry", 1).catch(() => true))
            continue;
        const text = `🐱 *Василий проголодался и ждёт тебя в Доме!*\n\n`
            + `Покорми его — забота каждый день приносит монеты.\n\n`
            + `[Покормить Василия](${(0, links_1.miniAppLink)(chatId, "game")})`;
        const ok = await push.sendPushSafely(chatId, "marketing_game", text, {
            dedupeKey: `pet-hungry:${today}`,
        });
        if (ok) {
            sent++;
            await (0, analytics_1.markFunnelSent)(chatId, "pet_hungry");
        }
    }
    logger_1.log.info({ sent, candidates: rows.length }, "[pet hungry push]");
 ...
 */
async function runPetEnergyPush(push) {
    const today = irkToday();
    const { rows } = await db_1.pool.query(`SELECT chat_id FROM clicker_state
    ...
    for (const r of rows) {
        const chatId = Number(r.chat_id);
        if (await (0, analytics_1.wasFunnelSent)(chatId, "energy_full", 1).catch(() => true))
            continue;
        const text = `⚡ *Энергия восстановилась — Василий готов тапать!*\n\n`
            + `Забеги за комбо дня.\n\n`
            + `[Играть](${(0, links_1.miniAppLink)(chatId, "game")})`;
        const ok = await push.sendPushSafely(chatId, "marketing_game", text, {
            dedupeKey: `pet-energy:${today}`,
        });
        if (ok) {
            sent++;
            await (0, analytics_1.markFunnelSent)(chatId, "energy_full");
        }
    }
    logger_1.log.info({ sent, candidates: rows.length }, "[pet energy push]");
    return { sent, candidates: rows.length };
}
