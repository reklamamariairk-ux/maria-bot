...
const signKey = crypto.createHmac("sha256", "MariaAppAuth").update(BOT_TOKEN).digest();
...
export async function completeAppLogin(chatId: number, phone: string): Promise<boolean> {
  ...
  const { rowCount } = await pool.query(
    `UPDATE app_login_nonces SET phone = $2
      WHERE chat_id = $1 AND phone IS NULL AND created_at > NOW() - INTERVAL '${NONCE_TTL_MIN} minutes'`,
    [chatId, clean]
  );
  return (rowCount ?? 0) > 0;
}
...
router.get("/auth/poll", async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT phone FROM app_login_nonces
      WHERE nonce = $1 AND created_at > NOW() - INTERVAL '${NONCE_TTL_MIN} minutes'`,
    [nonce]
  );
  if (!rows.length) { res.status(410).json({ ok: false, error: "expired" }); return; }
  if (!rows[0].phone) { res.json({ ok: true, status: "pending" }); return; }
  const phone = String(rows[0].phone);
  // Одноразовость: выдали токен — nonce сжигаем
  await pool.query(`DELETE FROM app_login_nonces WHERE nonce = $1`, [nonce]);
