...
import { searchCatalog, fetchProductById, Product } from "../scraper";
import { rateLimit } from "../middleware";

export interface CatalogDeps {
  getCatalog: () => Product[];
  catalogAge: () => string | null;
}
...
export function createCatalogRouter(deps: CatalogDeps): Router {
  ...
  router.get("/api/catalog/categories", rateLimit(120), (_req, res) => {
  ...
  router.get("/api/catalog/products", rateLimit(60), (req, res) => {
    const catalog = getCatalog();
  ...
  router.get("/api/catalog/search", rateLimit(30), (req, res) => {
    const q = String(req.query.q ?? "").trim();
    if (!q) {
      res.json({ products: [], total: 0 });
      return;
    }
    const products = searchCatalog(getCatalog(), q, 30);
    res.json({ products, total: products.length });
  });

  router.get("/api/catalog/product/:id", rateLimit(120), async (req, res) => {
