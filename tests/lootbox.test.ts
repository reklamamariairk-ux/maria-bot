...
describe("экономика кейса — новая таблица призов", () => {
  it("веса соответствуют заданным процентам", () => {
    ...
    expect(byKey.coins_zero).toBe(100);
    expect(byKey.coins_loss).toBe(300);
    expect(byKey.coins_slight_under).toBe(200);
    expect(byKey.coins_equal).toBe(120);
    expect(byKey.coins_plus).toBe(100);
    expect(byKey.coins_big).toBe(60);
    expect(byKey.coins_jackpot).toBe(10);
    expect(byKey.pigeon_common).toBe(50);
    expect(byKey.pigeon_rare).toBe(30);
    expect(byKey.pigeon_epic).toBe(5);
    expect(byKey.business_region + byKey.business_loyalty + byKey.business_manager + byKey.business_franchise).toBe(25);
  });
  ...
  it("при цене 100k базовая средняя отдача близка к ставке", () => {
    ...
    expect(ev).toBeCloseTo(93_000, -2);
  ...
  it("все призы принадлежат допустимым типам", () => {
    for (let i = 0; i < 5000; i++) {
      ...
      if (p.type === "coins") {
        ...
        expect(p.amount).toBeLessThanOrEqual(1_500_000);
