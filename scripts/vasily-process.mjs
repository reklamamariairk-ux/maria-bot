// Обработка арт-комплекта «Кондитерская карьера Василия»:
// коты: белый фон → альфа (flood-fill от краёв) → trim → единый рост → канвас 440×600;
// фоны: 768×1376 → 720w webp q80.
import sharp from "sharp";
import path from "node:path";
import fs from "node:fs";

const SRC = "C:/Users/user/AppData/Local/Temp/claude/C--Users-user/3de7e9c1-6cc3-42de-bbb4-37d54bbfc4e2/scratchpad/flow-assets";
...
async function cutoutCat(file) {
  ...
  // альфа: фон → 0; мягкий край: сосед фона → приглушить
  ...
  // масштаб: рост CONTENT_H, но не шире MAX_W
  ...
  await sharp({ create: { width: CANVAS_W, height: CANVAS_H, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } })
    ...
    .toFile(path.join(CAT_OUT, file));
  return `${file}: content ${cw}x${ch} -> ${nw}x${nh}`;
}
